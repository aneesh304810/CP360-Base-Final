...
def blob_metadata(scur, qtab, clob_col):
    scur.execute(f"""
        SELECT COUNT(*), MIN(l), MAX(l), ROUND(AVG(l), 1),
               STATS_MODE(l),
               ROUND(100 * SUM(CASE WHEN l = STATS_MODE(l) OVER ()
                     THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0), 2)
        FROM (SELECT DBMS_LOB.GETLENGTH("{clob_col}") l,
                     STATS_MODE(DBMS_LOB.GETLENGTH("{clob_col}"))
                       OVER () sm
              FROM {qtab}
              WHERE "{clob_col}" IS NOT NULL)""")
    cnt, lmin, lmax, lavg, lmode, uni = scur.fetchone()
    scur.execute(f"""
        SELECT COUNT(*) FROM {qtab}
        WHERE "{clob_col}" IS NOT NULL
          AND DBMS_LOB.GETLENGTH("{clob_col}") < :m""", m=lmode)
    truncated = scur.fetchone()[0]
    ...
    return dict(blob_count=cnt, len_min=lmin, len_max=lmax, len_avg=lavg,
                ...
                truncated_cnt=truncated, structure=structure)
