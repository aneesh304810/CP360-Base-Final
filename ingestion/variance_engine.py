...
def _salvage(scur, src, psel, items, manifest):
    """Batch died (typically one illegal column — e.g. ORA-00932 on a LOB):
    re-run per column so one offender can't zero out its neighbours.
    Returns a row aligned with `items`, None-filled for failing columns."""
    by_col = {}
    for i, m in enumerate(manifest):
        col = m[0] if isinstance(m, tuple) else m
        by_col.setdefault(col, []).append(i)
    out = [None] * len(items)
    bad = []
    for col, idxs in by_col.items():
        sub = ", ".join(items[i] for i in idxs)
        try:
            scur.execute(psel + sub + f" FROM {src}")
            vals = scur.fetchone()
    ...
    if bad:
        log.warning("  salvage: %d column(s) unprofileable, e.g. %s",
                    len(bad), bad[:2])
    return out
...
def _probe_exprs(col, masks, dtype="VARCHAR2"):
    ...
    if base not in _CHAR_TYPES:
        ...
        # LOB/LONG columns cannot be aggregate arguments (ORA-00932):
        # count via CASE, measure via DBMS_LOB.GETLENGTH.
        if "LOB" in base or base.startswith("LONG"):
            probes = {"TOTAL": "COUNT(*)",
                      "NONNULL": f"COUNT(CASE WHEN {c} IS NOT NULL "
                                 f"THEN 1 END)"}
            if "LOB" in base:
                probes["MAXLEN"] = f"MAX(DBMS_LOB.GETLENGTH({c}))"
            return probes
    ...
    e = {
        ...
        "MEDIAN":  (f"MEDIAN(TO_NUMBER({v} DEFAULT NULL ON CONVERSION ERROR))"),
        "MINV":    f"MIN({v})",
        "MAXV":    f"MAX({v})",
    }
...
def _risk_and_verdict(inferred, conf, s, declared):
    ...
    if inferred.startswith("MIXED"):
        return ("REVIEW_MIXED",
                "mixed content — multiple type populations in one column; "
...
def _classify_minority(scur, qtable, col, inferred, mask, masks):
    """For values that FAIL the dominant type: what do they conform to?
    Returns e.g. {"DATE:DD/MM/YYYY": 8, "DECIMAL": 2, "OTHER": 1} —
    distinguishes a second legitimate format (era split, alt vocabulary)
    from genuine garbage."""
    v = f'TRIM("{col.upper()}")'
    if inferred in ("INTEGER", "DECIMAL"):
        fail_cond = f"VALIDATE_CONVERSION({v} AS NUMBER) = 0"
    elif inferred.startswith("DATE:"):
        fail_cond = f"VALIDATE_CONVERSION({v} AS DATE, '{mask}') = 0"
    elif inferred == "BOOLEAN_FLAG":
        fail_cond = f"UPPER({v}) NOT IN ('Y','N','T','F','0','1')"
    else:
        return {}
    probes, names = [], []
    for m in masks:
        if m != mask:
            probes.append(f"SUM(VALIDATE_CONVERSION({v} AS DATE, '{m}'))")
            names.append(f"DATE:{m}")
    if not inferred.startswith(("INTEGER", "DECIMAL")):
        probes.append(f"SUM(VALIDATE_CONVERSION({v} AS NUMBER))")
        names.append("NUMERIC")
    probes.append(f"SUM(CASE WHEN UPPER({v}) IN ('Y','N','T','F','0','1') "
                  f"THEN 1 ELSE 0 END)")
    names.append("FLAG_VALUE")
    probes.append("COUNT(*)")
    names.append("_TOTAL_FAILING")
    try:
        scur.execute(
            "SELECT " + ", ".join(probes) +
            f" FROM {qtable} WHERE {v} IS NOT NULL AND {fail_cond}")
        row = scur.fetchone()
    except Exception as e:                                  # noqa: BLE001
        log.warning("minority census failed %s.%s: %s",
                    qtable, col, str(e)[:100])
        return {}
    out = {}
    total = 0
    for name, val in zip(names, row):
        if name == "_TOTAL_FAILING":
            total = val or 0
        elif val:
            out[name] = int(val)
    matched = sum(out.values())
    if total and total > matched:
        out["UNCLASSIFIED"] = int(total - matched)
    # keep only meaningful signals, largest first
    return dict(sorted(out.items(), key=lambda kv: -kv[1])[:5])


def _deviation_pass(scur, ccur, cconn, run_id, data_source, stage, tab,
                    qtab, per_col):
    """Second pass per table: >3-sigma outlier counts for numerics with known
    mu/sigma, approx NDV for all, and a top-values census for low-NDV columns.
    Updates the rows _classify already inserted."""
    numerics = [(c, st["MEAN"], st["STDDEV"]) for c, st in per_col.items()
                if st.get("MEAN") is not None and (st.get("STDDEV") or 0) > 0]
    # LOB / strongly-typed cols only have TOTAL/NONNULL probe keys;
    # NDV & value-census apply to content-probed character columns only
    others = [c for c, st in per_col.items() if "NUM_OK" in st]
    items, man = [], []
    for col, mu, sd in numerics:
        v = (f"TO_NUMBER(TRIM(\"{col.upper()}\") "
             f"DEFAULT NULL ON CONVERSION ERROR)")
        a = f"D{len(man)}"
        items.append(f"SUM(CASE WHEN ABS({v} - {mu}) > 3 * {sd} "
                     f"THEN 1 ELSE 0 END) AS {a}")
        man.append((col, "OUT"))
    for col in others:
        a = f"D{len(man)}"
        items.append(f'APPROX_COUNT_DISTINCT(TRIM("{col.upper()}")) AS {a}')
        man.append((col, "NDV"))
    out, ndv = {}, {}
    for b in range(0, len(items), 120):
        try:
            scur.execute("SELECT " + ", ".join(items[b:b + 120])
                         + f" FROM {qtab}")
            for (col, kind), val in zip(man[b:b + 120], scur.fetchone()):
                (out if kind == "OUT" else ndv)[col] = val
        except Exception as e:                              # noqa: BLE001
            log.warning("deviation pass failed %s: %s", qtab, str(e)[:100])
            return
    census = {}
    for col, n in ndv.items():
        if n and 2 <= n <= 12:
            try:
                scur.execute(f"""
                    SELECT val || ' \u00d7' || cnt FROM (
                      SELECT TRIM("{col.upper()}") val, COUNT(*) cnt
                      FROM {qtab} WHERE "{col.upper()}" IS NOT NULL
                      GROUP BY TRIM("{col.upper()}")
                      ORDER BY cnt DESC)
                    WHERE ROWNUM <= 6""")
                census[col] = " \u00b7 ".join(r[0] for r in scur.fetchall())
            except Exception:                               # noqa: BLE001
                pass
    for col in others:
        ccur.execute("""UPDATE recon_dtype_profile
            SET outlier_cnt = :o, ndv = :n, value_census = :v
            WHERE run_id = :r AND data_source = :ds AND stage = :st
              AND table_name = :t AND column_name = :c""",
            {"o": out.get(col), "n": ndv.get(col),
             "v": (census.get(col) or "")[:1000] or None,
             "r": run_id, "ds": data_source, "st": stage,
             "t": tab, "c": col})
    cconn.commit()
...
def _metric_exprs(col, dclass):
    c = f'"{col.upper()}"'
    if dclass == "LOB":
        return [("CNT", f"COUNT(CASE WHEN {c} IS NOT NULL THEN 1 END)"),
                ("NULLS", f"SUM(CASE WHEN {c} IS NULL THEN 1 ELSE 0 END)"),
                ("MAXLEN", f"MAX(DBMS_LOB.GETLENGTH({c}))")]
    if dclass == "RAWNUM":
            ...
            for i, ((stage, tab), colset) in enumerate(sorted(by_table.items()), 1):
                ...
                for b in range(0, len(cols), _BATCH):
                    ...
                    try:
                        ...
                        log.warning("probe batch failed %s (%s) — retrying "
                                    "per column", qtab, str(e)[:80])
                        row = _salvage(scur, src, psel, items, manifest)
                        if row is None:
                            continue
                    ...
                    for col, s in stats.items():
                        ...
                        fail_types = ({} if risk != "CAST_UNSAFE" else
                                      _classify_minority(scur, qtab, col,
                                                         inferred, mask, masks))
                        nn = s.get("NONNULL") or 0
                        ...
                        ccur.execute("""INSERT INTO recon_dtype_profile
                            (run_id, data_source, stage, table_name, column_name,
                             ...
                             mean_val, stddev_val, median_val, min_val, max_val,
                             ndv)
                            ...
                            [run_id, data_source, stage, tab, col,
                             ...
                             if (samples or fail_types) else None,
                             "Y" if is_pii else "N",
                             s.get("MEAN"), s.get("STDDEV"), s.get("MEDIAN"),
                             str(s.get("MINV"))[:80] if s.get("MINV")
                             is not None else None,
                             str(s.get("MAXV"))[:80] if s.get("MAXV")
                             is not None else None,
                             None])
                        cols_done += 1
                    ...
                    log.info("  -> %s: %d columns landed in recon_dtype_profile",
                             qtab, len(stats))
                    _deviation_pass(scur, ccur, cconn, run_id, data_source,
                                    stage, tab, qtab, stats)
            ...
            for i, ((stage, tab), pairs) in enumerate(sorted(per_table.items()), 1):
                ...
                for b in range(0, len(pairs), _BATCH):
                    ...
                    try:
                        ...
                        log.warning("metric batch failed %s (%s) — retrying "
                                    "per column", qtab, str(e)[:80])
                        row = _salvage(scur, qtab, psel, items,
                                       [(col, ch, m) for ch, col, m
                                        in manifest])
                        if row is None:
                            continue
...
def _summarize(ccur, cconn, run_id, data_source, chains):
    ...
    for (dwh_tab, _dwh_col, _s2t, _s2c), grp in groups.items():
        ...
        st["total"] += 1
        st["fg"] = grp[0]["fgroup"]
        fan = st.setdefault("fanout", defaultdict(set))
        for c in grp:
            fan[(c["STG1"][0], c["STG1"][1])].add(c["STG2"][1])
        broke = False
        for a, b in hops:
    ...
    for tab, s2 in tstats.items():
        ...
        # grain change: CLOB-explode chains (one STG1 column feeding many
        # STG2 columns) legitimately change row counts STG1->STG2 — annotate
        # rather than alarm when CNT there is the only break signal
        clob_explode = any(len(v) >= 3
                           for v in s2.get("fanout", {}).values())
        if (status == "RED" and clob_explode
                and set(s2["metric"]) <= {"CNT"}
                and set(s2["hop"]) <= {"STG1->STG2"}):
            status = "GRAIN"
