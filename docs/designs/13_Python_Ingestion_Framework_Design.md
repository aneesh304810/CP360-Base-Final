...
# Python Ingestion Framework — Design Document
...
```python
# loaders/base.py — the abstraction that prevents "one framework" degrading
# into three special cases
...
```python
# stamp.py — the 8 columns, applied uniformly. No feed exempts itself.
AUDIT_COLUMNS = {
...
# ROW_NUM makes a RAW row addressable to a physical line — essential for audit
# and for the quarantine resubmission path.
```

### Profile capture — sized for G4

```python
# profile.py — captured during the SAME pass as validation, never a second read
@dataclass
class LoadProfile:
    load_id: int
...
```python
# the framework does not open connections directly — it borrows from the
# shared pool (55). Pool size, not pod count, is the real fan-out ceiling.
