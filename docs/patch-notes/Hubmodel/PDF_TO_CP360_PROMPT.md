...
ARCHITECTURE FACTS (authoritative — do not contradict)
 ...
 • Two independent lanes: BATCH (Landing sFTP → Stage 1 RAW → Stage 2 Enriched →
   ...
   consumers, NO batch dependency). ALL API traffic routes through the API
   Gateway in BOTH directions: SEI APIs inbound, consumer API access to Hub
   data, and outbound loader submissions/acks to SEI (AD-11).
 ...
 • Physical tiers: Stage 1 RAW = Oracle, immutable append-only, 8 audit columns,
   ...
   latest-record) · Stage 3 Pre-Gold = standalone Oracle EXADATA —
   it exists for COMPLEX TRANSFORMATION (SCD2, facts, control-total tie-out;
   Smart Scan/HCC) · FINAL GOLD = the consumption layer: PBDW, IMDS, Pivotal,
   reached by publish-only SIMPLE MOVEMENT, no transformation in flight. The one cross-database hop is Stage 2 (Oracle) →
   Pre-Gold (Exadata): default DB-link + direct-path APPEND with HCC; Data Pump for
