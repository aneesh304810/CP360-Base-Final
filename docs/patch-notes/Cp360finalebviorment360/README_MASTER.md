...
Your LegacyLineage.jsx: NOW INCLUDED FULLY PATCHED (all 7 patches applied
to the complete source you provided: map spacing · DefModal popup · roomy
typography · ch-based smart widths · lineage-first order · XformStory band ·
inline overflow + wrap-safe proof). Deploy = overwrite ui/src/LegacyLineage.jsx.
The patch docs remain as the change log / review record only.
Mount edit + verification: LINEAGE_WIRING.md / LINEAGE_DEPLOYMENT.md.
Acceptance spec: lineage_final_mockup.html.
Deploy all three UI files together: LineageHome.jsx + BizLineage.jsx +
LegacyLineage.jsx, plus the one route edit.
Caveat: LegacyLineage was rebuilt from your uploaded source — diff against
your local copy before overwriting if it may have drifted.
