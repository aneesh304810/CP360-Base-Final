...
## v5 workbook (workload-grained — the new canonical)
sql/48_env_workload.sql creates ENV_WORKLOAD and DROPS env_infra_host (both
guarded). Ingest the v5 sheet directly (tsv/csv/xlsx): one row per monitorable
unit. Health_Check_Type drives probes: OpenShiftDeployment/Job -> per-workload
K8S readiness (ns/name) · HTTPS -> TLS on Health/SSL_Check_Target · TCP /
Database -> reachability · FileShare -> CIFS · NetworkRange / Descriptive /
Service / OpenShiftPlatform -> documented, not probed. Column-shifted rows are
auto-repaired (count in load log). env_infra remains the system-level
aggregate, so every UI contract is untouched; per-host export path returns
system-grained CSV now that env_infra_host is dropped.
