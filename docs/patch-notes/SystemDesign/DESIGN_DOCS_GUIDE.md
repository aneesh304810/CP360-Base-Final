...
## ```svg blocks — bring any diagram
A fenced svg block embeds raw SVG (draw.io export, PowerPoint save-as-SVG, D2/DOT
output, hand-authored) inline in a document, zoomable like mermaid:
    ```svg
    <svg viewBox="0 0 400 200">...</svg>
    ```

## Tier-1 editing (in the drill: ✏️ edit)
Toggle edit on any document: each section becomes a markdown editor with LIVE
preview (tables, mermaid, svg all render as you type). "⬇ download .md" / "⧉ copy"
exports the FULL document (front-matter + all sections, your edits applied) —
drop it into designs-md/, run build_design_docs.py (+ nightly design_docs step),
and the change reflects everywhere. Edits live only in the browser session until
exported: designs-md/ in git stays the single source of truth.
