...
/* ------------------------------------------------------------------ */
/* DefModal — centered popup shell for cramped contexts (map + rail). */
/* Table mode keeps its inline under-the-row panel. */
/* ------------------------------------------------------------------ */
...
function InlineDef({ t, system, dataSource, code, ctx, row, tableName, onDataSource,
 ...
 padding: roomy ? "10px 20px" : "7px 14px", background: mc }}>
 {def.master_name || "—"} · {def.field_code}
 <span style={{ float: "right", fontWeight: 400, opacity: 0.85 }}>{system} dictionary</span>
 </div>
 <div style={{ fontSize: roomy ? 18 : 14, fontWeight: 700, color: t.navy || "#10193b",
 ...
 <b style={{ fontWeight: 400, whiteSpace: "pre-line",
 ...
 fontSize: roomy ? 12 : 10.5, color: t.navy || "#10193b",
...
const CAP = 60;
 ...
 {dOpen ? "▾ definition open" : "click for the master-resolved definition"}</div>
 </div>
 </div>
 ...
 onDataSource={onDataSource} roomy />
 </DefModal>)}
 </React.Fragment>);
 })}
 {rows.length > CAP && (
...
export default function LegacyLineage({ t, system = "ADDVANTAGE", dataSource = "PBDW",
 ...
 borderRadius: 8, padding: "7px 10px", minWidth: "16ch", maxWidth: "28ch",
 background: "#fff",
...
const xfArrow = (xf) => (
 ...
 {p.stage}</div>
 <div style={{ fontFamily: "Roboto Mono, monospace",
 fontSize: 10.5, color: t.navy || "#10193b",
 wordBreak: "break-word" }}>
 {p.field_value == null ? "∅" : String(p.field_value)}</div>
 </div>))}
 </div>
...
const visEdges = net.edges.filter((e) => {
 ...
 onDataSource={jumpWarehouse} roomy />
 </DefModal>)}
 </>);
 })()}
 </div>
