...
const WL_TYPE = ["Deployment", "Job", "Hostname", "Service", "Descriptive",
 "IP/CIDR"];
const HC_TYPE = ["AirflowAPI", "HTTPS", "OpenShiftDeployment", "OpenShiftJob", "TCP",
 "Database", "FileShare", "Service", "Descriptive", "NetworkRange",
 "OpenShiftPlatform"];
const INV_VOCAB = {
 env: ["DEV", "SIT", "TRIAL_UAT", "PROD"],
 layer: ["Platform", "File System", "Database", "Consumer", "External API",
  "External SFTP"],
 zone: ["UNTRUSTED", "DMZ", "CORP", "APPLICATION", "DATA", "MGMT"],
 component: ["INTEGRATION_HUB", "FILE_SHARE", "DATA_HUB", "DATA_MOVEMENT",
  "CONSUMERS", "IDENTITY", "EXTERNAL"],
 direction: ["Inbound", "Outbound", "Bidirectional", "Internal"],
 growth: ["Minimal Growth", "Moderate Growth", "High Growth", "TBD"] };
  ...
  <div style={{ background: "#fff", border: `1px solid ${t.border}`,
   ...
   <table style={{ borderCollapse: "collapse", width: "100%", fontSize: 11 }}>
    ...
    {rows.map((r) => {
     ...
     const rowCells = (
      <tr key={key(r)} style={{ background: ed ? "#f0f7fc" : "transparent" }}>
       ...
       {FIELDS.map(([f]) => (
        <td key={f} style={{ ...td, ...mono, fontSize: 10 }}>
         {f === "ssl_expiry" && !r[f]
          ...
          : f === "ssl_expiry" && n != null && n <= 7
           ? <Chip tone="red">{r[f]} · {n}d</Chip>
           : String(r[f] || "").slice(0, 34) ||
             <span style={{ color: "#b9c4cd" }}>—</span>}
        </td>))}
       <td style={td}>
        <span onClick={() => { setEdit(ed ? null : key(r));
          setDraft(ed ? {} : { ...r }); }}
         style={{ fontSize: 9.5, fontWeight: 700, cursor: "pointer",
          padding: "2px 9px", borderRadius: 999,
          border: `1px solid ${t.border}`,
          background: ed ? t.navy : "#fff",
          color: ed ? "#fff" : t.sub }}>{ed ? "✕ close" : "✎ edit"}</span>
       </td>
      </tr>);
     return rowCells;
     /* modal editor rendered once, after the table */
   ...
   {edit && (() => {
    const r = rows.find((x) => key(x) === edit);
    if (!r) return null;
    const F = (label, f, kind, opts, monoIn) => (
     <div>
      <div style={{ fontSize: 8.6, fontWeight: 700, color: "#7d93a8",
       textTransform: "uppercase", letterSpacing: 0.6,
       marginBottom: 3 }}>{label}</div>
      {kind === "select" ? (
       <select value={draft[f] ?? r[f] ?? ""} onChange={(e) =>
         setDraft((d) => ({ ...d, [f]: e.target.value }))}
        style={{ width: "100%", fontSize: 11, padding: "6px 7px", height: 30,
       ...
       <textarea rows={2} value={draft[f] ?? r[f] ?? ""} onChange={(e) =>
         setDraft((d) => ({ ...d, [f]: e.target.value }))}
        style={{ width: "100%", fontSize: 10.5, padding: "6px 7px", ...mono,
         border: `1px solid ${t.border}`, borderRadius: 5,
         boxSizing: "border-box", resize: "vertical" }} />
      ) : (
       <input type={kind === "date" ? "date" : "text"}
        value={draft[f] ?? r[f] ?? ""} onChange={(e) =>
         setDraft((d) => ({ ...d, [f]: e.target.value }))}
        style={{ width: "100%", fontSize: 11, padding: "6px 7px", height: 30,
         ...(monoIn ? mono : {}), border: `1px solid ${t.border}`,
         borderRadius: 5, boxSizing: "border-box" }} />)}
     </div>);
    const Sec = ({ title, hint }) => (
     <div style={{ display: "flex", alignItems: "baseline", gap: 8,
      margin: "18px 0 10px", paddingBottom: 5,
      borderBottom: "1px solid #e8edf1" }}>
      <span style={{ fontSize: 9.6, fontWeight: 800, letterSpacing: 1.4,
       color: "#0f4775" }}>{title}</span>
      {hint && <span style={{ fontSize: 8.6, color: "#9aa7b2" }}>{hint}</span>}
     </div>);
    const grid = (cols, mt) => ({ display: "grid", gap: 12,
     marginTop: mt || 0, gridTemplateColumns: cols });
    const pillB = (extra) => ({ fontSize: 10.5, fontWeight: 700,
     padding: "5px 16px", borderRadius: 999, cursor: "pointer", ...extra });
    return (
     <div onClick={(e) => { if (e.target === e.currentTarget) {
        setEdit(null); setDraft({}); } }}
      style={{ position: "fixed", inset: 0, zIndex: 120,
       background: "rgba(16,25,59,.5)", display: "flex",
       alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 10, maxWidth: 820,
       width: "100%", maxHeight: "92vh", overflow: "auto",
       boxShadow: "0 24px 70px rgba(0,0,0,.35)" }}>
       <div style={{ display: "flex", alignItems: "center", gap: 10,
        padding: "14px 22px", background: t.navy,
        borderBottom: "3px solid #8a1f2d",
        borderRadius: "10px 10px 0 0" }}>
        <span style={{ fontSize: 9.6, color: "#8fa3b5",
         letterSpacing: 0.5 }}>INVENTORY / {r.env} /</span>
        <b style={{ fontSize: 13, color: "#fff" }}>{r.system_name}</b>
        <span onClick={() => { setEdit(null); setDraft({}); }}
         style={{ marginLeft: "auto", cursor: "pointer", color: "#8fa3b5",
          fontWeight: 800, fontSize: 14 }}>✕</span>
       </div>
       <div style={{ padding: "6px 22px 20px" }}>
        <Sec title="IDENTITY" hint="changing these creates a new identity (move) — probes & history re-key" />
        <div style={grid("1fr 1fr 1.4fr")}>
         {F("Environment", "env", "select", INV_VOCAB.env)}
         {F("Layer", "layer", "select", INV_VOCAB.layer)}
         {F("System name", "system_name")}
        </div>
        <Sec title="NETWORK & SECURITY" hint="drives probes, board lanes and the certificate register" />
        <div style={grid("1fr")}>
         {F("Hosts / endpoints · ; separated", "hosts", "area")}
        </div>
        <div style={grid("repeat(3,1fr)", 12)}>
         {F("Protocol / Port", "protocol_port", null, null, true)}
         {F("Direction", "direction", "select", INV_VOCAB.direction)}
         {F("SSL certificate expiry", "ssl_expiry", "date")}
        </div>
        <Sec title="CLASSIFICATION" hint="governs zone placement and board ordering" />
        <div style={grid("repeat(4,1fr)")}>
         {F("Project", "project")}
         {F("Zone", "zone", "select", INV_VOCAB.zone)}
         {F("Component", "component", "select", INV_VOCAB.component)}
         {F("Display order", "display_order")}
        </div>
        <Sec title="CAPACITY PLANNING" hint="" />
        <div style={grid("repeat(5,1fr)")}>
         {F("RAM", "sizing_ram")}
         {F("CPU", "sizing_cpu")}
         {F("Storage", "sizing_storage")}
         {F("Growth", "growth", "select", INV_VOCAB.growth)}
         {F("Hosting", "hosting")}
        </div>
        <Sec title="WORKLOADS & HEALTH COLLECTION"
         hint="one row per monitorable unit · Health_Check_Type drives the probe · all v5 columns" />
        {(() => {
         const wl = draft.workloads ?? r.workloads ?? [];
         const setW = (arr) => setDraft((d) => ({ ...d, workloads: arr }));
         const upd = (i2, f, v) => { const a = wl.map((x) => ({ ...x }));
          a[i2][f] = v; setW(a); };
         const wIn = (i2, f, mn) => (
          <input value={wl[i2][f] || ""} onChange={(e) =>
            upd(i2, f, e.target.value)}
           style={{ width: "100%", fontSize: 9.6, padding: "3px 5px",
            border: "1px solid #dfe6ec", borderRadius: 4,
            boxSizing: "border-box", ...(mn ? mono : {}) }} />);
         const wSel = (i2, f, opts) => (
          <select value={wl[i2][f] || ""} onChange={(e) =>
            upd(i2, f, e.target.value)}
           style={{ width: "100%", fontSize: 9.6, padding: 3,
            border: "1px solid #dfe6ec", borderRadius: 4 }}>
           <option value="" />
           {opts.map((o) => <option key={o} value={o}>{o}</option>)}
          </select>);
         const hcols = ["Namespace", "Workload_Name", "Workload_Type",
          "Host_or_Endpoint", "Endpoint_URL", "Health_Check_Type",
          "Health_Check_Target", "SSL_Check_Target", "Collection_Instruction"];
         return (
          <div>
           <div style={{ overflow: "auto", border: "1px solid #e8edf1",
            borderRadius: 6 }}>
            <table style={{ borderCollapse: "collapse", minWidth: 1180,
             width: "100%" }}>
             <thead><tr>
              {hcols.map((c) => <th key={c} style={{ textAlign: "left",
               fontSize: 8, letterSpacing: 0.4, color: "#7d93a8",
               padding: "6px 7px", borderBottom: "1.5px solid #e4ebf1",
               background: "#fafcfc",
               textTransform: "uppercase" }}>{c}</th>)}
              <th style={{ background: "#fafcfc",
               borderBottom: "1.5px solid #e4ebf1" }} />
             </tr></thead>
             <tbody>
              {wl.map((x, i2) => (
               <tr key={i2}>
                <td style={{ padding: "4px 5px" }}>{wIn(i2, "namespace", 1)}</td>
                <td style={{ padding: "4px 5px" }}>{wIn(i2, "workload_name", 1)}</td>
                <td style={{ padding: "4px 5px", minWidth: 96 }}>
                 {wSel(i2, "workload_type", WL_TYPE)}</td>
                <td style={{ padding: "4px 5px", minWidth: 150 }}>
                 {wIn(i2, "host", 1)}</td>
                <td style={{ padding: "4px 5px", minWidth: 150 }}>
                 {wIn(i2, "endpoint_url", 1)}</td>
                <td style={{ padding: "4px 5px", minWidth: 130 }}>
                 {wSel(i2, "hc_type", HC_TYPE)}</td>
                <td style={{ padding: "4px 5px", minWidth: 150 }}>
                 {wIn(i2, "hc_target", 1)}</td>
                <td style={{ padding: "4px 5px", minWidth: 130 }}>
                 {wIn(i2, "ssl_target", 1)}</td>
                <td style={{ padding: "4px 5px", minWidth: 210 }}>
                 {wIn(i2, "collection_instruction")}</td>
                <td style={{ padding: "4px 6px", cursor: "pointer",
                 color: "#c1113a", fontSize: 10 }}
                 onClick={() => setW(wl.filter((_, k3) => k3 !== i2))}>🗑</td>
               </tr>))}
             </tbody>
            </table>
           </div>
           <div style={{ marginTop: 7 }}>
            <span onClick={() => setW([...wl, { namespace: "",
              workload_name: "", workload_type: "Hostname", host: "",
              endpoint_url: "", hc_type: "TCP", hc_target: "",
              ssl_target: "", collection_instruction: "" }])}
             style={{ fontSize: 9, fontWeight: 700, padding: "3px 11px",
              borderRadius: 999, border: `1px solid ${t.border}`,
              cursor: "pointer", color: t.sub }}>＋ Add workload</span>
           </div>
          </div>);
        })()}
        <Sec title="NOTES" hint="" />
        <div style={grid("1fr")}>
         {F("Status / notes", "notes", "area")}
        </div>
       </div>
       <div style={{ display: "flex", gap: 10, alignItems: "center",
        padding: "14px 22px", borderTop: "1px solid #e8edf1",
        background: "#fafcfd", borderRadius: "0 0 10px 10px" }}>
        <span onClick={() => del(r)} style={pillB({
         border: "1px solid #e4a09a", color: "#c1113a" })}>🗑 Delete row</span>
        <span style={{ marginLeft: "auto" }} />
        <span onClick={() => { setEdit(null); setDraft({}); }}
         style={pillB({ border: `1px solid ${t.border}`,
          color: t.sub })}>Cancel</span>
        <span onClick={() => save(r)} style={pillB({ background: t.navy,
         color: "#fff", padding: "5px 22px" })}>Save changes</span>
       </div>
      </div>
     </div>);
   })()}
...
export function NetworkBoard({ t, env = "DEV" }) {
 ...
 useEffect(() => {
  ...
  const onWheel = (e) => { if (!e.ctrlKey) return; e.preventDefault();
   setZoom((z) => zClamp(z + (e.deltaY < 0 ? 0.15 : -0.15))); };
  el.addEventListener("wheel", onWheel, { passive: false });
  return () => el.removeEventListener("wheel", onWheel);
 }, [fs]);   // eslint-disable-line
 useEffect(() => { api.envInfraTopology(envSel).then((tp) => {
  setTopo(tp);
  if (tp && !tp.error) {                  // multi-member boxes start expanded
   const o = {};
   [...(tp.nodes || []), ...NBV_STATIC].forEach((n) => {
    const src = n.sub || n.hosts || "";
    if (n.id === "app.hub" || src.split(";").filter((x) => x.trim()
        && !x.trim().startsWith("ns:")).length > 1) o[n.id] = true;
   });
   setOpen(o);
  }
 }); }, [envSel]);
 ...
 const zBtn = { fontSize: 11, fontWeight: 800, minWidth: 26, height: 24,
  borderRadius: 6, border: `1px solid ${t.border}`, background: "#fff",
  color: "#41566b", cursor: "pointer", textAlign: "center", lineHeight: "22px",
  userSelect: "none", padding: "0 6px" };
 return (
  <div style={fs ? { position: "fixed", inset: 0, zIndex: 99, background: "#fff",
   ...
   <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap",
    ...
    <span onClick={() => setZoom((z) => zClamp(z - 0.2))} style={zBtn}>−</span>
    <span style={{ fontSize: 10, color: t.sub, minWidth: 38, textAlign: "center",
     ...mono }}>{Math.round(zoom * 100)}%</span>
    <span onClick={() => setZoom((z) => zClamp(z + 0.2))} style={zBtn}>＋</span>
    <span onClick={zFit} style={zBtn}>⤧ fit</span>
    <span onClick={() => setZoom(1)} style={zBtn}>1:1</span>
    <span onClick={() => { const nf = !fs; setFs(nf);
      if (nf) setTimeout(zFit, 0); else setZoom(1); }}
     style={{ ...zBtn, marginLeft: "auto" }}>
     {fs ? "✕ exit full screen" : "⤢ full screen"}</span>
      ...
      return (
       <g key={l.id}>
        <path d={d} fill="none" stroke={col} strokeWidth={1.6}
         ...
         style={l.tbd ? undefined : { animation: `nbFlow ${
          (worstOf(a) === "DOWN" || worstOf(b) === "DOWN") ? 2.4 : 1.1
          }s linear infinite` }} />
